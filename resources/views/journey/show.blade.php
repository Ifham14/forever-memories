    ...
    <div class="min-h-screen flex flex-col justify-between bg-cover bg-center relative"
        ...
        <div class="flex-grow flex items-center justify-center relative z-10 py-20">
            <div class="w-full max-w-5xl bg-white rounded-lg shadow-lg p-15 space-y-6">
                <h1 class="text-2xl font-bold text-gray-800">{{ $journey->title }}</h1>

                <div class="mt-4 text-sm text-gray-600 flex items-center space-x-4">
                    <span class="inline-block mt-1 bg-blue-100 text-blue-800 text-xs px-3 py-1 rounded-full">{{ $journey->status ?? 'Draft' }}</span>
                ...
                <p class="text-sm text-gray-700 leading-relaxed">
                    {{ $journey->story ?? 'No story provided.' }}
                </p>

                <div>
                    <h2 class="text-lg font-bold text-gray-800 mb-2">Memories</h2>
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                        @forelse ($journey->images as $image)
                            <div class="overflow-hidden rounded-lg border border-gray-200">
                                <img src="{{ asset('storage/' . $image->image_path) }}" alt="Memory Image" class="w-full h-auto object-cover">
                            </div>
                        @empty
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-800 mb-2">Order of Service</h2>
                    @php
                        $orderOfServiceFields = [
                    ...
                    <div class="space-y-5 text-sm text-gray-700">
                        @foreach ($orderOfServiceFields as $field => $label)
                            <div class="flex items-start justify-between gap-4">
                                <div class="w-1/3 font-semibold">{{ $label }}</div>
                                <div class="w-2/3 font-medium">{{ $journey->$field ?? 'Not Specified' }}</div>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-800 mb-2">Appreciations</h2>
                    @php
                        $appreciations = [
                            'honorary_pallbearers' => 'Honorary Paul Bearers',
                    ...
                    <div class="space-y-5 text-sm text-gray-700">
                        @foreach ($appreciations as $field => $label)
                            <div class="flex items-start justify-between gap-4">
                                <div class="w-1/3 font-semibold">{{ $label }}</div>
                                <div class="w-2/3 font-medium">
                                    {{ $journey->$field ?? 'Not Specified' }}
                                </div>
                            </div>
                        @endforeach
                    </div>
